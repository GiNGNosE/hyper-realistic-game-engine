...
- Create an ADR for decisions changing interfaces, data contracts, determinism model, baseline policy,
...
- Cycle evidence bundling derives touched ADR IDs from git history over the configured cycle window
  (`CYCLE_WINDOW_DAYS`, default `84`) and optional explicit override (`CYCLE_ADR_IDS`).
